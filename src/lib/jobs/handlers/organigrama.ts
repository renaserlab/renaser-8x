...
export async function handleEstudioOrganigrama(job: Job) {
  ...
  const [{ data: c }, { data: claims }, { data: parts }, { data: puestos }, { data: hallazgos }, { data: procesos }] = await Promise.all([
    ...
    sb.from("org_puestos").select("nombre,mision,decide,persona,respaldo,reporta_a").eq("company_id", job.company_id),
  ...
  const actuales = (puestos ?? []).map((p) => `- ${p.nombre}${p.persona ? ` — lo ocupa ${p.persona}` : " — VACANTE"}${p.mision ? ` · misión: ${p.mision}` : ""}${p.respaldo ? "" : " · SIN RESPALDO"}`);
  ...
  const contexto = comoDato(
    ...
    [
      ...
      actuales.length ? `EL ORGANIGRAMA ACTUAL EN 8X:\n${actuales.join("\n")}` : "ORGANIGRAMA ACTUAL: aún no dibujado.",
  ...
  return { puestos_por_construir: r.data.puestos_por_construir.length };
}
