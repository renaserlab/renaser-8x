...
> **Estado 2026-08-23 (fase de integración real): CONECTADO Y VERIFICADO.** Proyecto remoto `otqfqafstrohugvgbkmd` (`.env.local`, no versionado). `supabase/schema.sql` aplicado completo vía Management API (`POST /v1/projects/{ref}/database/query`). `npm run test:supabase` → **24/24 PASS** (AUTH, RLS A/B, dueño→empleado DENY, vistas, tokens/canje, RPC anon/authenticated DENY + service_role ALLOW, findings, storage firmado, transacción con rollback, take_job concurrente + recover, idempotencia, company_stats/handle_new_user, Realtime al consultor).
>
> Fallos reales encontrados y corregidos en esta fase: (1) `schema.sql` tenía 12 `$` perdidos (`do $` / `$$` → `do $ begin`) por un script de parcheo previo (`String.replace` trata `$` como patrón); Postgres rechazaba el archivo en la línea 275. Restaurados; el test estático `p0-schema` no lo detectaba (solo busca fragmentos) — ahora el archivo está validado por ejecución real. (2) El test de Realtime disparaba el UPDATE en el mismo instante del `SUBSCRIBED`; el servidor registra la suscripción unos cientos de ms después y el evento se perdía. Verificado con script independiente que el consultor autenticado sí recibe eventos (RLS correcta); se corrigió el test (espera 1,5 s). (3) `afterAll` del test no borraba las filas de `public.users` (sin FK a `auth.users`): corregido. Ningún cambio de arquitectura.
